...
def prepare_rouge_results(p, r, f, metric):
    return '\t{}:\t{}: {:5.2f}\t{}: {:5.2f}\t{}: {:5.2f}\n'.format(metric, 'P', 100.0 * p, 'R', 100.0 * r, 'F1',
                                                                   100.0 * f)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='allowed metrics are: jaccard, bleu, rouge, meteor, bertscore, tfidf, use, sbert')
    parser.add_argument('predFile', type=str, default=None)
    parser.add_argument('refFile', type=str, default=None)
    parser.add_argument('--metric', dest='metric', type=str, default='bertscore')
    ...
    metric_list = ['jaccard', 'bleu', 'rouge', 'meteor', 'bertscore', 'tfidf', 'use', 'sbert']
    if metric not in metric_list:
        print('{} not an available metric yet. Please use one of the following metrics: {}'.format(metric, [m for m in
                                                                                                            metric_list]))
